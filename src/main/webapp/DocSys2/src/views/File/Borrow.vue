<template>
  <div>
    <Breadcrumb :style="{margin: '24px 0'}">
      ...
      <BreadcrumbItem>出借档案</BreadcrumbItem>
    </Breadcrumb>
    <Content :style="{padding: '24px', minHeight: '500px', background: '#fff'}">
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
        ...
        <Row>
          <Col span="8">
            <FormItem label="借档原因" prop="reason">
              <Input v-model="formValidate.reason" placeholder="请输入借档原因">></Input>
            </FormItem>
          </Col>
          <Col span="8">
        ...
        <Row>
          ...
          <Col span="8">
            <FormItem>
              <Button type="primary" @click="goSave('formValidate')" :disabled="dis">出借</Button>
  ...
  data () {
    ...
    return {
      ...
      ruleValidate: {
        ...
        reason: [
          { required: true, message: '借档原因为必填项', trigger: 'blur' }
        ],
        source: [
        ...
        if (valid) {
          ...
          axios.get(API.borrow, {
            ...
            if (res.data === 'OK') {
              ...
              this.$Notice.success({
                title: '操作完成!',
                desc: '档案：' + this.formValidate.code + '已出借！'
              })
              setTimeout(() => {
